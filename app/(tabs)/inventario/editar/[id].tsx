...
import { ArrowLeft, Package, DollarSign, Info, MapPin, Camera, X, Check } from 'lucide-react-native'
import { safeHaptics } from '@/core/utils/haptics'
import { ForwardLogo, RequirePermission, PremiumInput, PremiumButton, GlassCard, ConfirmModal } from '@/core/ui'
...
export default function EditarProductoScreen() {
  ...
  // S4 (ADR-6): resolve roles from auth store; gate cost field and edit affordances
  const user = useAuthStore((s) => s.user)
  const userRoles = (user?.roles as string[] | undefined) ?? []
  const userCanViewCost = canViewCost(userRoles)
  
  const {
    loadingData, isPending, isUploading,
    nombre, setNombre,
  ...
  useFocusEffect(
    ...
    }, [id, queryClient, resetFormState, bumpFocusKey, productosKeys])
  )
  ...
  if (loadingData) {
    return (
      <View style={{ flex: 1, backgroundColor: colors.bg, justifyContent: 'center', alignItems: 'center' }}>
        ...
        <Text style={{ marginTop: 20, color: colors.textMuted, fontFamily: 'Outfit_700Bold' }}>SINCRONIZANDO DATA...</Text>
      </View>
    )
  }

  // S4 FIX #1 (ADR-6 / Decision #81): product-edit is cost-roles only.
    ...
    >
      ...
      >
            ...
            >
              {imageUri ? (
                ...
                <View style={{ alignItems: 'center' }}>
                  <View style={{ width: 48, height: 48, borderRadius: 24, backgroundColor: 'rgba(255,255,255,0.05)', alignItems: 'center', justifyContent: 'center', marginBottom: 12 }}>
                    <Camera size={24} color="#fff" opacity={0.5} />
                  </View>
                  <Text style={{ color: colors.textMuted, fontSize: 10, fontFamily: 'Outfit_800ExtraBold', opacity: 0.7, textTransform: 'uppercase', letterSpacing: 1 }}>CAMBIAR ASSET</Text>
                </View>
              )}
            </TouchableOpacity>
          </MotiView>
        ...
        <MotiView from={{ opacity: 0, translateY: 20 }} animate={{ opacity: 1, translateY: 0 }} transition={{ delay: 200 }} style={{ marginTop: 10 }}>
          <Text style={styles.subSectionTitle}>EMPAQUE (OPCIONAL)</Text>
          ...
          <Text style={styles.subSectionTitle}>FRACCIÓN MÍNIMA DE VENTA</Text>
          <View style={{ flexDirection: 'row', gap: 10, marginBottom: 12 }}>
            {FRACCIONES.map((f) => {
              ...
              return (
                ...
                >
                  <Text style={[styles.unitChipText, selected && { color: '#fff' }]}>
                    {f.label}
                  </Text>
                </TouchableOpacity>
              )
                ...
                <View style={{ flexDirection: 'row', gap: 8, marginTop: 8 }}>
                  {blisterContenido ? (
                    <TouchableOpacity
                      ...
                      style={{ paddingHorizontal: 12, paddingVertical: 8, borderRadius: 10, backgroundColor: 'rgba(128,128,128,0.08)', borderWidth: 1, borderColor: 'rgba(128,128,128,0.15)' }}
                    >
                      <Text style={{ fontSize: 11, fontFamily: 'Outfit_600SemiBold', color: 'rgba(128,128,128,0.7)' }}>
        ...
        <MotiView from={{ opacity: 0, translateY: 20 }} animate={{ opacity: 1, translateY: 0 }} transition={{ delay: 300 }} style={{ marginTop: 10 }}>
          <Text style={styles.subSectionTitle}>COSTOS E INVENTARIO</Text>
          ...
          {userCanViewCost && (
            <>
              ...
              >
                <GlassCard style={styles.priceWidget}>
                  <View>
                    <Text style={styles.priceLabel}>VALOR SUGERIDO DE SALIDA</Text>
                    <Text style={styles.priceValue}>
                      ${precioVentaSugerido.toLocaleString('es-AR', { minimumFractionDigits: 2 })}
                    </Text>
                  </View>
                  <View style={styles.priceIconContainer}>
                    <DollarSign size={24} color="#fff" strokeWidth={3} />
                  </View>
                </GlassCard>
              </MotiView>
              ...
              {parseFloat(precioCompra) > 0 && (
                <View style={styles.roiRow}>
                  <View style={styles.roiChip}>
                    ...
                    <Text style={[styles.roiValue, { color: grossProfit >= 0 ? '#00c19e' : '#ef4444' }]}>
                      ${grossProfit.toLocaleString('es-AR', { minimumFractionDigits: 2 })}
                    </Text>
                  </View>
                  <View style={styles.roiChip}>
                    <Text style={styles.roiLabel}>ROI</Text>
                    <Text style={[styles.roiValue, { color: roiPercentage >= 0 ? '#00c19e' : '#ef4444' }]}>
          ...
          <View style={{ marginBottom: 20 }}>
            <Text style={styles.fieldLabel}>UBICACIÓN EN ALMACÉN</Text>
              ...
              {UBICACIONES.map((loc) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.locChip,
                      selected && { backgroundColor: 'rgba(0, 193, 158, 0.15)', borderColor: colors.primary }
                    ]}
                  >
                    <Text style={[styles.locChipText, selected && { color: colors.primary }]}>
                      {loc.label === 'Ninguna' ? <MapPin size={12} color={selected ? colors.primary : "#fff"} opacity={0.5} /> : loc.label}
                    </Text>
                  </TouchableOpacity>
                )
              })}
        ...
        <MotiView from={{ opacity: 0, translateY: 20 }} animate={{ opacity: 1, translateY: 0 }} transition={{ delay: 400 }} style={styles.footer}>
          {/* S4 (ADR-6): save action visible only to cost-authorized roles */}
          {userCanViewCost && (
            <RequirePermission module="MOD_STOCK" action="update">
              <PremiumButton
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { color: 'rgba(128,128,128,0.9)', fontSize: 9, fontFamily: 'Outfit_900Black', textTransform: 'uppercase', letterSpacing: 2 },
  subSectionTitle: { fontSize: 11, fontFamily: 'Outfit_800ExtraBold', color: 'rgba(128,128,128,0.8)', textTransform: 'uppercase', marginBottom: 16, letterSpacing: 1 },
  unitChip: { flex: 1, paddingVertical: 14, borderRadius: 16, alignItems: 'center', backgroundColor: 'rgba(128,128,128,0.08)', borderWidth: 1, borderColor: 'rgba(128,128,128,0.15)' },
  unitChipText: { fontFamily: 'Outfit_700Bold', fontSize: 13, color: 'rgba(128,128,128,0.7)' },
  ...
  priceWidget: { padding: 20, marginBottom: 8, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderLeftWidth: 4, borderLeftColor: '#00c19e' },
  priceLabel: { color: '#00c19e', fontSize: 9, fontFamily: 'Outfit_900Black', textTransform: 'uppercase', letterSpacing: 1.5 },
  priceValue: { color: '#00c19e', fontSize: 32, fontFamily: 'Outfit_900Black', marginTop: 4 },
  priceIconContainer: { width: 48, height: 48, borderRadius: 24, backgroundColor: 'rgba(0, 193, 158, 0.2)', alignItems: 'center', justifyContent: 'center' },
  ...
  fieldLabel: { color: 'rgba(128,128,128,0.8)', fontSize: 9, fontFamily: 'Outfit_800ExtraBold', textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 12, marginLeft: 4 },
  locChip: { paddingHorizontal: 16, paddingVertical: 12, borderRadius: 14, backgroundColor: 'rgba(128,128,128,0.08)', borderWidth: 1, borderColor: 'rgba(128,128,128,0.15)' },
  locChipText: { fontSize: 12, fontFamily: 'Outfit_600SemiBold', color: 'rgba(128,128,128,0.7)' },
